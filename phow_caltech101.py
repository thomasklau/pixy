...
class Configuration(object):
    ...
    def setImagePath(self, imagePath):
        self.calDir = imagePath
    def setNumTrain(self, numTrain):
        self.numTrain = numTrain
        self.imagesperclass = self.numTrain + self.numTest
    def setNumTest(self, numTest):
...
def getImageDescriptor(model, im, conf, vocab):
    im = standarizeImage(im)
    height, width = im.shape[:2]
    numWords = vocab.shape[1]

    frames, descrs = getPhowFeatures(im, conf.phowOpts)
    # quantize appearance
    if model.quantizer == 'vq':
        binsa, _ = vq(descrs.T, vocab.T)
...
def getFeatures(i, all_images, conf):
    im = imread(all_images[i])
    return getPhowFeatures(im, conf.phowOpts)[1]
